...
**解决的问题**：对每个 ABS 用户对（ABS-pair），决定使用哪种传输模式
...
- 决定每个 ABS-pair 应该使用卫星直达还是ABS中继
- 决定ABS中继时应使用NOMA还是OMA
- 优化目标：**最大化系统总速率**

### 关键设计：基于 ABS-pairs 决策（路线A）

**重要**：模式选择基于 **ABS 配对**（ABS-pairs），而非卫星配对（SAT-pairs）

**原因**：
- NOMA/OMA 转发是 ABS 的物理行为
- ABS 的资源分配（功率、带宽）基于 A2G 信道增益 Γ^d
- ABS-pairs 根据 Γ^d 排序形成，与 SAT-pairs（基于卫星信道增益 Γ^s）可能不同

**物理一致性**：
```
卫星侧: SAT-pairs（基于 Γ^s） → 用于 S2A 解码
ABS侧:  ABS-pairs（基于 Γ^d） → 用于 NOMA/OMA 模式选择
```

这确保了不会对"不存在的配对"计算虚假收益（详见 `docs/route_a_abs_pairing_mode_selection_rationale.md`）。

### 算法原理

**贪心策略**：对每个 ABS-pair 独立选择最优模式

```
对于每个 ABS-pair (weak_user, strong_user):  # 基于 Γ^d 配对
    计算4种模式的收益（pair总速率）:
        gain_sat     = R_sat[weak] + R_sat[strong]
        gain_noma    = R_noma[weak] + R_noma[strong]    # ABS NOMA速率
        gain_oma_w   = R_oma[weak] + R_sat[strong]      # ABS OMA速率
...
**注意**：`R_noma` 和 `R_oma` 是基于 ABS-pairs 和 A2G 信道增益计算的，已包含 S2A 瓶颈约束。
...
while 未收敛:
    ...
    1. 计算信道增益和配对（基于当前p_d）
       - 计算 A2G 信道增益（Γ^d）
       - 计算 S2A 信道增益
       - 计算 SAT-pairs（基于 Γ^s，用于卫星NOMA和S2A解码）
       - 计算 ABS-pairs（基于 Γ^d，用于ABS NOMA/OMA）

    2. [步骤1] Greedy模式选择（基于 ABS-pairs）
       - 输入：sat_rates, abs_noma_rates, abs_oma_rates, abs_pairs
       - 输出：modes（每个 ABS-pair 的最优模式）

    3. [步骤2] KKT资源分配
       - 输入：sat_pairs, modes, A2G速率, S2A信道增益
       - 输出：b_allocated（每个 SAT-pair 的S2A带宽）
       - 注：S2A解码基于 SAT-pairs，但带宽分配考虑 modes（基于 ABS-pairs）
...
```python
# 1. 卫星侧配对（SAT-pairs，基于卫星信道增益 Γ^s）
sat_pairs, _ = allocator.optimal_user_pairing(sat_channel_gains)
# 用途：
#   - 卫星 NOMA 下行传输（卫星→用户直达）
#   - S2A 链路解码（卫星→ABS，DF中继的第一跳）

# 2. ABS侧配对（ABS-pairs，基于A2G信道增益 Γ^d）
abs_pairs, _ = allocator.optimal_user_pairing(channel_gains_a2g)
# 用途：
#   - ABS NOMA/OMA 下行传输（ABS→用户，DF中继的第二跳）
#   - 模式选择决策（基于 ABS-pairs）
```
...
1. **物理独立性**：
   - 卫星信道增益 Γ^s ≠ A2G 信道增益 Γ^d（不同路径损耗）
   - 最优配对应基于实际传输链路的信道增益

2. **NOMA 传输要求**：
   - 强用户需要解码弱用户信号（SIC）
   - 配对必须基于该链路的真实信道增益才能保证 SIC 可行

3. **模式选择的物理一致性**（路线A的核心）：
   - NOMA/OMA 是 ABS 的发射行为，必须基于 ABS-pairs
   - 否则会对"不存在的物理动作"进行优化

**数据流示意**：
```
用户 → 卫星信道(Γ^s) → SAT-pairs → 卫星NOMA速率(R_sat)
                                   ↓
                              S2A解码(基于SAT-pairs和β^s)
                                   ↓
     ← A2G信道(Γ^d) ← ABS-pairs ← S2A速率(R_s2a)
                         ↓
                    模式选择(基于ABS-pairs)
                         ↓
                    ABS NOMA/OMA传输(基于ABS-pairs和β^d)
```

这确保了：
- A2G NOMA 传输的最优性（强用户正确解码弱用户信号）
- 模式选择的物理一致性（不对虚假配对计算收益）
- 整体系统的端到端性能优化
...
**修改模块**：[src/optimizers/mode_selector.py](../src/optimizers/mode_selector.py)

**核心变更**：将模式选择的决策单元从 SAT-pairs 改为 ABS-pairs

### 修改前后对比

| 方面 | 修改前 | 修改后（路线A） |
|------|--------|----------------|
| **函数签名** | `select_modes(..., sat_pairs)` | `select_modes(..., abs_pairs)` |
| **遍历对象** | `for (w,s) in sat_pairs` | `for (w,s) in abs_pairs` |
| **决策依据** | 基于卫星配对 | 基于ABS配对 |
| **物理一致性** | ❌ 可能对不存在的配对决策 | ✅ 决策与ABS物理行为一致 |
...
原实现在 SAT-pair (0,1) 上计算 `abs_noma_rates[0] + abs_noma_rates[1]`，但：
- ABS 实际不会对 (0,1) 进行 NOMA 转发
- ABS 的 NOMA 对象是 (0,2) 或 (1,3)
- 这个"收益"在物理上是**虚构的**

**解决方案**（路线A）：
- 模式选择遍历 ABS-pairs
- 每个 ABS-pair 的决策对应 ABS 的实际物理行为
- 保证优化目标与物理实现一致

### 影响范围

**修改的文件**：
1. ✅ [src/optimizers/mode_selector.py](../src/optimizers/mode_selector.py) - 三个选择器的参数和实现
2. ✅ [test_route_a.py](../test_route_a.py) - 验证测试

**无需修改的文件**（已经正确传入 abs_pairs）：
- [src/satcon_system.py](../src/satcon_system.py)
- [src/optimizers/position_optimizer.py](../src/optimizers/position_optimizer.py)
- [experiments/compare_abs_positions_3d.py](../experiments/compare_abs_positions_3d.py)

### 验证结果

测试场景：SAT-pairs ≠ ABS-pairs
- ✅ 所有选择器正确基于 ABS-pairs 决策
- ✅ 贪心和穷举达到相同最优解
- ✅ 边界情况测试通过

详细说明见：[docs/route_a_abs_pairing_mode_selection_rationale.md](route_a_abs_pairing_mode_selection_rationale.md)
